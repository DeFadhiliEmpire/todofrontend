...
function App() {
  ...
  const fetchTasks = async (token) => {
    const response = await fetch("https://todobacken.onrender.com/tasks", {
  ...
  const deleteTask = async (id) => {
    await fetch(`https://todobacken.onrender.com/tasks/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const updateTaskStatus = async (id, currentStatus) => {
    ...
    const response = await fetch(
      `https://todobacken.onrender.com/tasks/${id}/status`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const updateTaskPriority = async (id, newPriority) => {
    const response = await fetch(
      `https://todobacken.onrender.com/tasks/${id}/priority`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const MainApp = () => (
    <div className="min-h-screen bg-blue-50 flex flex-col">
      <nav className="bg-blue-500 text-white px-6 py-4 flex justify-between items-center shadow-md">
        ...
        <button
          onClick={logout}
          className="px-4 py-2 bg-green-500 hover:bg-red-600 text-white font-bold rounded-full shadow transition-colors duration-200"
        >
          Logout
        </button>
      </nav>
